...
struct Kmer
{
  ...
  inline void set_base(uint32_t index, uint8_t ch) {
    ...
    cout << "index / kCharsPerWord=" << index / kCharsPerWord << endl;
    cout << "ch=" << ch << endl;
    cout << "(word_t(ch) << offset)=" << (word_t(ch) << offset) << endl;
    cout << " (m_data[index / kCharsPerWord] & ~(word_t(3) << offset))=" <<  (m_data[index / kCharsPerWord] & ~(word_t(3) << offset)) << endl;
  }
