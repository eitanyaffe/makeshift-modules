...
void KCube::add_snp_vector(string seq, int min_segment, int min_count, vector<int>& result)
{
  ...
  index_t mask1 = 1 << m_ksize;
  index_t mask2 = 2 << m_ksize;
  index_t mask3 = 3 << m_ksize;

  int segment_length = 0;
  for (int coord=0; coord<length-m_ksize+1; ++coord) {
    index_t index = seq_v[coord];
    int count = get_kmer(index);
    int count1 = get_kmer(index ^ mask1);
    int count2 = get_kmer(index ^ mask2);
    int count3 = get_kmer(index ^ mask3);
    bool present = count >= min_count && count >= count1 && count >= count2 && count >= count3;
    if (present) {
      segment_length++;
    } else {
      if (segment_length >= min_segment)
	for (int scoord=(coord-segment_length); scoord<coord; ++scoord)
	  flags[scoord+m_ksize/2-1] = true;
      segment_length = 0;
    }
  }
  if (segment_length >= min_segment)
    for (int scoord=(length-segment_length); scoord<length-m_ksize+1; ++scoord)
      flags[scoord+m_ksize/2-1] = true;

  for (int coord=0; coord<length; ++coord)
    result[coord] += flags[coord];
}
