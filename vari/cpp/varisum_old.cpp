...
void init_params(int argc, char **argv, Parameters& params)
{
  ...
  params.add_parser("ofn_nt_cov", new ParserFilename("output nt coverage file"), true);
  params.add_parser("ofn_link_cov", new ParserFilename("output link coverage file"), true);
  params.add_parser("odir_nt_cov", new ParserFilename("output nt coverage dir"), true);
  params.add_parser("odir_link_cov", new ParserFilename("output link coverage dir"), true);
...
void save_tables(map<string, int>& contig_map,
		 ...
		 string ofn, string ofn_cov, string odir_cov)
{
  cout << "writing variance table: " << ofn << endl;
  ...
  cout << "writing united coverage table: " << ofn_cov << endl;
  ofstream out_ucov(ofn_cov.c_str());
  massert(out_ucov.is_open(), "could not open file %s", ofn_cov.c_str());
  out_ucov << "contig\tcoord\tcount" << endl;
  ...
  for (map<string, int>::iterator it=contig_map.begin(); it!=contig_map.end(); ++it) {
    ...
    massert((unsigned int)length == total_contig.size(), "internal error");
    ...
    string ofn_cov = odir_cov + "/" + contig;
    cout << "writing coverage table: " << ofn_cov << endl;
    ofstream out_cov(ofn_cov.c_str());
    massert(out_cov.is_open(), "could not open file %s", ofn_cov.c_str());
    out_cov << "contig\tcoord\tcount" << endl;
    for (unsigned int coord = 0; coord < total_contig.size(); ++coord) {
      out_cov << contig << "\t" << coord+1 << "\t" << total_contig[coord] << endl;
      out_ucov << contig << "\t" << coord+1 << "\t" << total_contig[coord] << endl;
    }
    out_cov.close();
  }
  out.close();
...
int main(int argc, char **argv)
{
  ...
  string odir_nt_cov = params.get_string("odir_nt_cov");
  string odir_link_cov = params.get_string("odir_link_cov");

  string ofn_nt_cov = params.get_string("ofn_nt_cov");
  string ofn_link_cov = params.get_string("ofn_link_cov");
  ...
  save_tables(contig_map, nt_coord, total_nt, ofn_nt, ofn_nt_cov, odir_nt_cov);
  save_tables(contig_map, link_coord, total_link, ofn_link, ofn_link_cov, odir_link_cov);
}
