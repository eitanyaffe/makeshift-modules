...
if ($#ARGV == -1) {
	print STDERR "usage: $0 <ifn> <query_table_ifn> <target_table_ifn> <query type aa|nt> <ofn>\n";
	exit 1;
}

my ($ifn, $query_table_ifn, $target_table_ifn, $query_type, $ofn) = @ARGV;

my $query_length_field = $query_type eq "aa" ? "aa_length" : "length";
...
while (my $line = <IN>) {
    ...
    my @f = split("\t", $line);
    my $gene = $f[$h{gene}];
    my $length = $f[$h{$query_length_field}];
