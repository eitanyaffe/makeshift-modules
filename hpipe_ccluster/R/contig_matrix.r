...
plot.contig.matrix=function(adir, fdir, aid, ids, ver, titles)
{
    tt = NULL
    for (i in 1:length(ids)) {
        id = ids[i]
        title = titles[i]
        tt = rbind(tt, data.frame(id=id, load.table(paste(adir, "/datasets/", id, "/ccluster_base/cmatrix_stats", sep=""))))
    }
