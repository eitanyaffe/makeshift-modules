...
#    ylim = range(1, max(c(x$inter, x$face)))
    ...
    fig.start(fdir=fdir, ofn=paste(fdir, "/raw_", id, ".pdf", sep=""), type="pdf", width=4, height=4)
...
#    lines(x$log_start, x$inter, col="blue",lwd=lwd)
    ...
    fig.start(fdir=fdir, ofn=paste(fdir, "/norm_", id, ".pdf", sep=""), type="pdf", width=4, height=4)
    ...
    fig.start(fdir=fdir, ofn=paste(fdir, "/contigs_", id, ".pdf", sep=""), type="pdf", width=4, height=4)
    plot(x$log_start, x$contig_count, log="y", type="l", main=paste("contigs",id),lwd=lwd)
    fig.end()

    # distrib
    fig.start(fdir=fdir, ofn=paste(fdir, "/distrib_", id, ".pdf", sep=""), type="pdf", width=4, height=4)
    plot.new()
    ylim = c(0.00001, 10)
...
plot.summary=function(distal.threshold, adir, mreads, aid, ids, cmat.version, titles, fdir, ymax)
{
    ...
    for (i in 1:length(ids)) {
        ...
        ifn = paste(adir, "/datasets/", id, "/ccluster_base/", xname, "/cis_decay", sep="")
        x = load.table(ifn)
