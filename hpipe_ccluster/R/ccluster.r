plot.marginals=function(contigs.ifn, contacts.ifn, threshold, fdir)
{
    ...
    ofn = paste(fdir, "/contig_marginals.pdf", sep="")
...
plot.complete.graph=function(contigs.ifn, contacts.ifn, anchor.ifn, known.ifn, fdir)
{
    ...
    system(paste("mkdir -p", fdir))
    ofn = paste(fdir, "/contig_graph_anchors.pdf", sep="")
    ...
    ofn = paste(fdir, "/contig_graph_known.pdf", sep="")
...
plot.cluster.tree=function(tree.ifn, contigs.ifn, cluster.table.ifn, fdir)
{
    ...
    system(paste("mkdir -p", fdir))
    ofn = paste(fdir, "/contig_tree.pdf", sep="")
    cat(sprintf("generating plot: %s\n", ofn))
    pdf(ofn, 10, 10)
    plot.new()
    ...
    x = contigs[order(contigs$order),]
    ofn = paste(fdir, "/contig_tree_contig_size.pdf", sep="")
    cat(sprintf("generating plot: %s\n", ofn))
    pdf(ofn, 10, 4)
    barplot(x$length/10^3, col=x$col, las=2, ylab="kb", border=x$col)
    dev.off()

    ofn = paste(fdir, "/contig_tree_marginal.pdf", sep="")
    cat(sprintf("generating plot: %s\n", ofn))
    pdf(ofn, 10, 4)
    barplot(x$marginal, col=x$col, las=2, ylab="marginal", border=x$col)
    dev.off()
}
