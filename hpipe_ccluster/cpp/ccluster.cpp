...
int update_neighbors(Node& p, Node& l, Node& r, Node& n, double min_score, int max_contig_pairs, ScoreType stype, bool correct_cluster_size)
{
  ...
  double N_n = n.contigs.size();

  // TBD: switch to a more conservative approach by including N_n
  // double N = N_l + N_r + N_n;
  double N = N_l + N_r;
  ...
  double w_l = N_l/(N_l+N_r);
  double w_r = N_r/(N_l+N_r);
...
void usage(const char* name, UserParams& params)
{
  ...
  cout << " -min_length <int>: minimal contig length" << endl;
...
void read_contigs(string fn, vector<Contig>& contigs, map<string, int>& contig_map, map<string, set<string> >& marginals,
		  int min_coverage, int min_length)
{
  ...
  massert(length_idx != -1, "length column missing in contig file");
  while(in) {
    split_line(in, fields, delim);
    if(fields.size() == 0)
      break;
    ...
    if ((int)marginals[name].size() < min_coverage || length < min_length)
      continue;
...
int main(int argc, char **argv)
{
  ...
  read_contigs(params.contig_ifn, g_contigs, contig_map, marginals, params.min_coverage, params.min_length);
  massert(g_contigs.size() > 0, "no contigs pass filter");
  cout << "min contig marginal coverage: " << params.min_coverage << endl;
  cout << "min contig length: " << params.min_length << endl;
  cout << "number of selected contigs: " << g_contigs.size() << endl;
