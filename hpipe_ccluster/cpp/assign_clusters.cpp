...
vector<string> set_contigs(Node& node, map<int, Node>& nodes, map<int, string>& contig_map, map<string, int>& contig_length_map)
{
  ...
  if (node.leaf) {
    ...
    vector<string> lc = set_contigs(left, nodes, contig_map, contig_length_map);
    vector<string> rc = set_contigs(right, nodes, contig_map, contig_length_map);

    contigs.insert(contigs.end(), lc.begin(), lc.end());
    contigs.insert(contigs.end(), rc.begin(), rc.end());
  }

  // update total contig length
  node.nt_length = 0;
  for (unsigned int i=0; i<contigs.size(); ++i) {
    string contig = contigs[i];
    massert(contig_length_map.find(contig) != contig_length_map.end(), "contig not found in length map");
    node.nt_length += contig_length_map[contig];
  }
  return contigs;
}

void set_clusters(Node& node, map<int, Node>& nodes, int min_elements, int min_length, double min_score,
		  map<string, int>& contig_clusters, int& cluster, int level)
{
  ...
  if ((node.nt_length < min_length) || ((int)node.contigs.size() < min_elements) || (node.score/node.contigs.size() > min_score/100)) {
  ...
  if (((int)left.contigs.size() >= min_elements) && (left.nt_length >= min_length))
    set_clusters(left, nodes, min_elements, min_length, min_score, contig_clusters, cluster, level+1);

  if (((int)right.contigs.size() >= min_elements) && (right.nt_length >= min_length))
    set_clusters(right, nodes, min_elements, min_length, min_score, contig_clusters, cluster, level+1);
}
...
void usage(const char* name, UserParams& params)
{
  ...
  cout << " -min_lenth <int>: minimal total length of contigs in cluster (nt)" << endl;
...
void read_contigs(string fn, map<int, string>& contig_map, map<string, int>& contig_length_map, vector<string>& contigs)
{
...
int main(int argc, char **argv)
{
  ...
  vector<string> contigs;
  map<string, int> contig_length_map;

  // init contigs and contacts
  cout << "contig table: " << params.contig_ifn << endl;
  read_contigs(params.contig_ifn, contig_map, contig_length_map, contigs);
  ...
  for (unsigned int i=0; i<roots.size(); i++) {
    ...
    set_contigs(node, g_nodes, contig_map, contig_length_map);
  }
  ...
  for (unsigned int i=0; i<roots.size(); i++) {
    Node& node = get_node(roots[i]);
    if (((int)node.contigs.size() >= params.min_elements) && (node.nt_length > params.min_length))
      set_clusters(node, g_nodes, params.min_elements, params.min_length, params.min_score, contig_clusters, cluster, 0);
  }
  massert(cluster-1 > 0, "no clusters found");
