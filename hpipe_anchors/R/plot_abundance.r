plot.abundance=function(ifn.coverage, ifn.seeds, ifn.ca, ifn.gc, fdir)
{
    ...
    fig.start(fdir=fdir, type="pdf", ofn=paste(fdir, "/all.pdf", sep=""), width=width.dense, height=height.dense)
    par(yaxs="i")
    ...
    fig.start(fdir=fdir, type="pdf", ofn=paste(fdir, "/anchored.pdf", sep=""), width=width.dense, height=height.dense)
    par(yaxs="i")
    ...
    fig.start(fdir=fdir, type="pdf", ofn=paste(fdir, "/anchors.pdf", sep=""), width=width.dense, height=height.dense)
    ...
    # clean
    fig.start(fdir=fdir, type="pdf", ofn=paste(fdir, "/gc_abundance_all.pdf", sep=""), width=width.scatter, height=height.scatter)
    plot(df$a, df$gc, xlab="abundance", ylab="GC", pch=".", col=1, cex=1)
    fig.end()

    # color anchors
    ix = is.element(df$contig, ca$contig)
    fig.start(fdir=fdir, type="pdf", ofn=paste(fdir, "/gc_abundance_anchors.pdf", sep=""), width=width.scatter, height=height.scatter)
    ...
    fig.start(fdir=fdir, type="pdf", ofn=paste(fdir, "/gc_abundance_anchor_seeds.pdf", sep=""), width=width.scatter, height=height.scatter)
    plot(df$a, df$gc, xlab="abundance", ylab="GC", pch=".", col="gray", cex=1)
    points(df$a[ix], df$gc[ix], xlab="abundance", ylab="GC", pch=".", col="darkred", cex=2)
    fig.end()
}
