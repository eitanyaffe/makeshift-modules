...
model.predict=function(
    ...
    fends.ifn="/home/eitany/storage/o3c/results/h1_cl_gc_1000.binned",
    ...
    mfields.fns=NULL,
    cell.indices=NULL, noise.indices=NULL, prior.cell=NULL,
    scope, model, prior,
    ofields.x=c("f1", "f2"), ofields.y=c("f1", "f2"))
{
  ...
  ofn=paste(tmp.dir, "/", get.short.fn(fends.ifn), ".", index, sep="")
  log=paste(log.dir, "/", get.short.fn(fends.ifn), ".", index, sep="")

  # basic parameters
  command = sprintf("%s -fends %s -from_fend %d -to_fend %d -output %s", binary, fends.ifn, from.fend, to.fend, ofn)
  ...
  # cellular model
  if (model == "cellular") {
      command = paste(command, "-prior_cell", prior.cell,
          paste("-noise_field", noise.indices, collapse=" "),
          paste("-cell_field", cell.indices, collapse=" "))
  }
    ...
    ofields.y=c("frag_len_bin","frag_gc_bin"), from.vals.y=c(1,1), to.vals.y=c(5,5), Rcall)
{
  ...
  for (i in 1:nrow(ranges)) {
    ...
    command = sprintf("%s %s R/model_predict.r model.predict ofields.x=\"%s\" ofield.x.from=\"%s\" ofield.x.to=\"%s\" ofields.y=\"%s\" ofield.y.from=\"%s\" ofield.y.to=\"%s\" index=%d from.fend=%d to.fend=%d fends.ifn=\"%s\" log.dir=\"%s\" tmp.dir=\"%s\" binary=\"%s\" prior=%g scope=\"%s\" model=\"%s\"",
        ...
        r$index, r$from.fend, r$to.fend, fends.ifn, log.dir, tmp.dir,
        binary, prior, scope, model)

    if (model == "cellular") {
        command = sprintf("%s  prior.cell=%g noise.indices=\"%s\" cell.indices=\"%s\"",
            command, prior.cell, vec2str(noise.indices), vec2str(cell.indices))
    }
    ...
    ofns = c(ofns, paste(tmp.dir, "/", get.short.fn(fends.ifn), ".", r$index, sep=""))
  }
  ...
  system(paste("rm -rf ", tmp.dir, get.short.fn(fends.ifn), "*", sep=""))

  system(paste("rm -rf ", log.dir, get.short.fn(fends.ifn), "*", sep=""))
  system(paste("rm -rf ", tmp.dir, get.short.fn(fends.ifn), "*", sep=""))
...
{
  fends.ifn = paste(prefix, ".", fends.suffix, sep="")
  table = load.table(fends.ifn)
  fends = table$fend
  model = "count"
  ...
  model.predict.split(tmp.dir=tmp.dir, log.dir=log.dir, wd=wd, binary=binary,
                      fends.ifn=fends.ifn, ofn=ofn, fends=fends, num.splits=num.splits,
...
compute.expected.counts=function(
    ...
    model.prefix.cell="/home/eitany/storage/o3c/results/h1_cl_gc_1000_1M",
    ...
    omit.x.zero=F, omit.y.zero=F, Rcall)
{
  ...
  prior = read.delim(prior.ifn, header=F)[1,1]

  if (model == "cellular") {
      prior.ifn = paste(model.prefix.cell, "prior", sep=".")
      cat(sprintf("reading prior file %s\n", prior.ifn))
      prior.cell = read.delim(prior.ifn, header=F)[1,1]
  } else {
      prior.cell = 0
  }
  ...
  if (model == "cellular") {
      mtable.noise = mtable[-match("anchor", mtable$raw_field),]
      mtable.cell = mtable[-match("abundance", mtable$raw_field),]
      mtable.noise$prefix = model.prefix
      mtable.noise$compartment = "noise"
      mtable.cell$prefix = model.prefix.cell
      mtable.cell$compartment = "cell"
      mtable = rbind(mtable.noise, mtable.cell)
      mfields = mtable$field
      mfields.maxvals = mtable$size
      mfields.fns = paste(paste(mtable$prefix, mtable$field, sep="_"), ".f", sep="")
      cell.indices = which(mtable$compartment == "cell" & mtable$raw_field != "anchor")
      noise.indices = which(mtable$compartment == "noise")
  } else {
  ...
  model.predict.split(tmp.dir=tmp.dir, log.dir=log.dir, wd=wd, binary=binary,
                      ...
                      cell.indices=cell.indices, noise.indices=noise.indices, prior.cell=prior.cell,
