...
compute.prior=function(count.table, fields, functions, min.probability=10^-100)
{
    table = append.func.to.counts(count.table, fields, functions)
    N = dim(table)[1]
    table$product = rep(1.0, N)
    for (i in 1:length(fields))
        table$product = table$product * table[,fields[i]]
    ...
    optimal = sum(table$count) / sum(table$product)
    middle = (top + bottom) / 2
...
learn.model.file=function(model.prefix, model.params, fields)
{
  ...
  Pr = compute.prior(count.table=count.table, fields=fields, functions=functions)
