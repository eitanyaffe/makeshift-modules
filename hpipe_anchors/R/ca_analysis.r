...
plot.inter.anchor.matrix=function(ifn, ifn.order, min.contacts, min.enrichment, fdir)
{
    ...
    vtable = table[table$o > 0 & table$anchor_src < table$anchor_tgt,]
    ...
    vtable$log.e = log10(vtable$e)
    vtable$log.o = log10(vtable$o)
    lim = c(5, max(c(vtable$o, vtable$e)))
    fig.start(ofn=paste(fdir, "/anchor_anchor_scatter.pdf", sep=""), fdir=fdir, width=4, height=4.5, type="pdf")
    plot.new()
    plot.window(xlim=lim, ylim=lim, log="xy")
    title(ylab="log10(obs)", xlab="log10(exp)")
    grid()
    abline(a=0, b=1, lty=2)
    points(vtable$e, vtable$o, pch=19, cex=0.1)
    box()
    axis(1, las=2)
    axis(2, las=2)
    fig.end()
    ...
    main = paste("N=", N, ", cor=", round(cc, 3), sep="")
