...
// model
enum Model { m_count, m_std, m_cellular };
...
public:
  ...
  double prior_cell;
  vector<int> noise_fields;
  vector<int> cell_fields;
...
ostream& operator <<(ostream &os, const UserParams &params)
{
  ...
  if (params.model == m_cellular) {
    os << "cellular prior: " << params.prior_cell << endl;
    os << "noise fields:";
    for (unsigned int i=0; i<params.noise_fields.size(); i++)
      os << " "  << params.m_features.fields[params.noise_fields[i]];
    os << endl;
    os << "cellular fields:";
    for (unsigned int i=0; i<params.cell_fields.size(); i++)
      os << " "  << params.m_features.fields[params.cell_fields[i]];
    os << endl;
  }
...
void parse_user_arguments(int argc, char **argv, UserParams& params)
{
  ...
  while (i < argc)
	...
	params.prior = atof(arg);
      else if (option == "-prior_cell")
	params.prior_cell = atof(arg);
	...
	params.o_features_y.size = atoi(arg);
      else if (option == "-noise_field")
	params.noise_fields.push_back(atoi(arg)-1);
      else if (option == "-cell_field")
	params.cell_fields.push_back(atoi(arg)-1);
      else {
	cerr << "Error: unknown option: " << option << ", i=" << i << endl;
	exit(1);
      }
...
double fends_expected(const vector<Fend>& fends_x, const vector<Fend>& fends_y,
		      ...
		      const vector<int>& msizes,
                      const double prior,
                      const double prior_cell,
		      const vector<int>& noise_fields,
		      const vector<int>& cell_fields,
		      const int anchor_matrix_index)

{
  ...
  for(jt_x = fends_x.begin(); jt_x != fends_x.end(); jt_x++)
	...
	} else if (model == m_cellular) {

	  // noise portion
	  double value_noise = prior;
	  for(unsigned int ii = 0; ii < noise_fields.size(); ii++) {
	    int mi = noise_fields[ii];
	    value_noise *= model_matrices[mi][MATRIX_INDEX(fend_x.levels[mi], fend_y.levels[mi], msizes[mi])];
	  }

	  // cell portion
	  int anchor;
	  if (fend_x.on_anchor && !fend_y.on_anchor)
	    anchor = fend_x.anchor;
	  else if (!fend_x.on_anchor && fend_y.on_anchor)
	    anchor = fend_y.anchor;
	  else if (fend_x.on_anchor && fend_y.on_anchor && (fend_x.anchor == fend_y.anchor))
	    anchor = fend_x.anchor;
	  else
	    anchor = 0;
	  double value_cell = 0;
	  if (anchor != 0) {
	    value_cell = prior_cell;
	    const int ami = anchor_matrix_index;
	    value_cell *= model_matrices[ami][MATRIX_INDEX(anchor, anchor, msizes[ami])];
	    for(unsigned int ii = 0; ii < cell_fields.size(); ii++) {
	      int mi = cell_fields[ii];
	      value_cell *= model_matrices[mi][MATRIX_INDEX(fend_x.levels[mi], fend_y.levels[mi], msizes[mi])];
	    }
	  }
	  value = value_noise + value_cell;
	} else {
...
int main(int argc, char **argv)
{
  ...
  int anchor_matrix_index = -1;
  if (params.model == m_cellular) {
	  massert(params.prior > 0 && params.prior_cell > 0 && params.noise_fields.size() > 0 && params.cell_fields.size(), "for cellular model please define cell_prior,and one or more cell_fields and cell_noises");
	  for(unsigned int ii = 0; ii < params.noise_fields.size(); ii++)
	    massert(params.noise_fields[ii]>=0 && params.noise_fields[ii] < params.m_features.size, "noise_field %d out of range", params.noise_fields[ii]+1);
	  for(unsigned int ii = 0; ii < params.cell_fields.size(); ii++)
	    massert(params.cell_fields[ii]>=0 && params.cell_fields[ii] < params.m_features.size, "cell_field %d out of range", params.cell_fields[ii]+1);

	  anchor_matrix_index = find(params.m_features.fields.begin(), params.m_features.fields.end(), "anchor") - params.m_features.fields.begin();
	  cout << "anchor matrix index: " << anchor_matrix_index << endl;
  }
    ...
    for (unsigned int iy=0; iy<keys_y.size(); iy++)
	  ...
	  continue;
        ...
        double total_common = fends_expected(fends_common, fends_common, params.scope, params.model,
					     ...
					     params.prior,
					     params.prior_cell, params.noise_fields, params.cell_fields, anchor_matrix_index);
