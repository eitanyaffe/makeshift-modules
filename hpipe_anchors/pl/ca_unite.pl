...
if ($#ARGV == -1) {
	print STDERR "usage: $0 <observed table> <expected table> <expected cell table> <contig bins> <anchor bins> <ofn>\n";
	exit 1;
}

my $ifn_o = $ARGV[0];
my $ifn_contigs = $ARGV[1];
my $ifn_contigs_cell = $ARGV[2];
my $ifn_contig_bins = $ARGV[3];
my $ifn_anchor_bins = $ARGV[4];
my $ofn = $ARGV[5];
...
print STDERR "reading contig cellular expected: $ifn_contigs_cell\n";
open(IN, $ifn_contigs_cell) || die;
$header = <IN>;
%h = parse_header($header);
...
while (my $line = <IN>) {
    ...
    $matrix{$anchor}->{$contig}->{contig_expected_cell} = $exp;
}
close(IN);
...
open(OUT, ">", $ofn) || die;
print OUT "contig\tcontig_anchor\tanchor\tcontig_total_count\tcontig_coverage\tanchor_contig_count\tfend_count\tcontig_expected\tcontig_expected_cell\tany_observed\n";
...
foreach my $contig (keys %{$matrix{$anchor}}) {
    ...
    print OUT $matrix{$anchor}->{$contig}->{contig_expected}, "\t", $matrix{$anchor}->{$contig}->{contig_expected_cell}, "\t";
    print OUT $matrix{$anchor}->{$contig}->{any_observed}, "\n";
} }
close(OUT);
