...
if ($#ARGV == -1) {
	print STDERR "usage: $0 <gene table> <gene->contig table> <min identity> <go tree> <ofn.prefix>\n";
	exit 1;
}

my $ifn = $ARGV[0];
my $ifn_genes = $ARGV[1];
my $min_identity = $ARGV[2];
my $ifn_go_tree = $ARGV[3];
my $ofn_prefix = $ARGV[4];
...
print OUT_P "id\tcount\tcontig_count\tdesc\n";

open(OUT_F, ">", $ofn_function) || die $ofn_function;
print OUT_F "id\tcount\tcontig_count\tdesc\n";

open(OUT_C, ">", $ofn_component) || die $ofn_component;
print OUT_C "id\tcount\tcontig_count\tdesc\n";

foreach my $id (keys %tree) {
    $tree{$id}->{gene_count} = scalar keys %{$tree{$id}->{genes}};
    $tree{$id}->{contig_count} = scalar keys %{$tree{$id}->{contigs}};
}
...
foreach my $id (sort { $tree{$b}->{gene_count} <=> $tree{$a}->{gene_count} } keys %tree) {
    ...
    my $contig_count = scalar keys %{$tree{$id}->{contigs}};
    next if ($gene_count == 0);
    if ($type eq "biological_process") {
	print OUT_P $id, "\t", $gene_count, "\t", $contig_count, "\t", $tree{$id}->{desc}, "\n";
    }
    if ($type eq "molecular_function") {
	print OUT_F $id, "\t", $gene_count, "\t", $contig_count, "\t", $tree{$id}->{desc}, "\n";
    }
    if ($type eq "cellular_component") {
	print OUT_C $id, "\t", $gene_count, "\t", $contig_count, "\t", $tree{$id}->{desc}, "\n";
    }
}
close(OUT_P);
close(OUT_C);
