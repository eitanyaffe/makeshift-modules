...
if ($#ARGV == -1) {
	print STDERR "usage: $0 <gene table> <background gene table> <background gene table desc field> <ofn> <ofn backtable> <whole words> <poor annotation descriptions>\n";
	exit 1;
}
...
while (my $line = <IN>) {
    ...
    next if (!defined($gene2desc{$gene}));
