...
if ($#ARGV == -1) {
	print STDERR "usage: $0 <input dir> <contig table> <similarity mask table> <offset> <output dir contacts> <output dir masked> <output file>\n";
	exit 1;
}

my ($idir, $icontig, $isim, $offset, $odir_contact, $odir_masked, $ofn) = @ARGV;
...
open(OUT, ">", $ofn) || die $ofn;
print OUT "contig1\tcontig2\tcontacts\tmasked_contacts\n";
