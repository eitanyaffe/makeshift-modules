...
select.contigs=function(ifn, min.pearson, min.zscore, max.discard.fraction, ofn.contigs, ofn.bins)
{
    df = load.table(ifn)
    df$selected = df$pearson >= min.pearson & df$zscore >= min.zscore

    # compute fraction of filtered contigs per bin
    tt = table(df$bin, df$selected)
