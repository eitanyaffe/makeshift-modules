...
void multi_contig(int& segment_index, ofstream& out_segments, ofstream& out_contigs,
		  string contig, vector < Coverage >& covs, double outlier_fraction,
		  double min_P, double pseudo_count, string weight_style, int min_center_seg_len)
{
  ...
  for (unsigned int i=0; i<segments.size(); ++i) {
    ...
    out_segments << segments[i].start+1 << "\t" << segments[i].end << "\t" << (segments[i].is_center ? "F" : "T") << endl;
    segment_index++;
  }
...
void break_multi_init_params(const char* name, int argc, char **argv, Parameters& params)
{
...
{
  Parameters params;
  break_multi_init_params(name, argc, argv, params);
  ...
  vector<string> contigs = covs[0].get_contigs();
  ...
  for (unsigned int i=0; i<contigs.size(); ++i) {
    // cout << contigs[i] << endl;
    multi_contig(segment_index, out_segments, out_contigs, contigs[i], covs, outlier_fraction,
