...
void Dissolve::set_counts(string contig, vector < Coverage >& covs)
{
...
void Dissolve::get_segments_basic(vector<Segment>& segments)
{
  ...
  for (set<int>::iterator it=m_center_coords.begin(); it != m_center_coords.end(); ++it) {
	...
	segments.push_back(Segment(center_start, prev_center_coord+1, true));
      center_start = center_coord;

      // outlier segment
      if (center_coord > 0)
	segments.push_back(Segment(prev_center_coord+1, center_coord, false));
    }
  ...
  if (prev_center_coord+1 < m_contig_length)
    segments.push_back(Segment(prev_center_coord+1, m_contig_length, false));
}

void Dissolve::get_segments(vector<Segment>& segments, int min_center_seg_len)
{
  vector<Segment> basic_segments;
