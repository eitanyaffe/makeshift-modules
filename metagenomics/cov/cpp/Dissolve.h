...
struct Segment {
  int start, end;
  bool is_center;
  Segment(int _start, int _end, bool _is_center) : start(_start), end(_end), is_center(_is_center) {};
};
...
class Dissolve {
 private:
  ...
  void get_segments_basic(vector<Segment>& segments);
 ...
 public:
  ...
  void set_counts(string contig, vector < Coverage >& covs);
  ...
  void get_segments(vector<Segment>& segments, int min_center_segment_length);
};
