			  ...
			  map<string, vector<int> >& coverage)
{
  ...
  while(in) {
    ...
    map< int, map <Variation, int> >& poly_contig = poly[contig];
...
void read_library_table(string fn, vector< Library >& libs)
{
  ...
  // parse header
  split_line(in, fields, delim);
  int lib_ind = get_field_index("lib", fields);
  int path_ind = get_field_index("path", fields);
  ...
  while(in) {
    ...
    string id = fields[lib_ind];
    string path = fields[path_ind];
    Library lib(id, path);
    libs.push_back(lib);
  }
}
...
void save_output(vector< Library >& libs,
		 map<string, int>& contig_map,
		 map<string, string>& contig_seq,
		 vector < map< string, map< int, map <Variation, int> > > >& table_libs,
		 vector < map<string, vector<int> > >& cov_libs,
		 string ofn)
{
  ...
  for (map<string, int>::iterator it=contig_map.begin(); it!=contig_map.end(); ++it) {
    ...
    map< int, set < Variation > > keys;

    // collect all relevant entries
    for (unsigned int i=0; i<libs.size(); ++i) {
      map< string, map< int, map <Variation, int> > >& table = table_libs[i];
      if (table.find(contig) == table.end())
	continue;
      map< int, map <Variation, int> >& table_contig = table[contig];
      for (map< int, map <Variation, int> >::iterator xt = table_contig.begin(); xt != table_contig.end(); ++xt) {
	int coord = (*xt).first;
	map <Variation, int>& xmap = (*xt).second;
	for (map <Variation, int>::iterator jt=xmap.begin(); jt!=xmap.end(); ++jt) {
	  Variation var = (*jt).first;
	  keys[coord].insert(var);
	}
      }
    }

    // output entries
    for (map< int, set < Variation > >::iterator xt = keys.begin(); xt != keys.end(); ++xt) {
      int coord = (*xt).first;
      set < Variation >& vars = (*xt).second;

      // get ref nt
      char ref_nt = contig_seq[contig][coord];
      
      // output coverage
      out << contig << "\t" << coord+1 << "\t" << "REF" << "\t" << ref_nt;
      for (unsigned int i=0; i<libs.size(); ++i) {
	map<string, vector<int> >& cov = cov_libs[i];
	vector<int>& cov_contig = cov[contig];
	out << "\t" << cov_contig[coord];
      }
      out << endl;
      
      // output variations
      for (set < Variation >::iterator jt = vars.begin(); jt != vars.end(); ++jt) {
	Variation var = *jt;
	out << contig << "\t" << coord+1 << "\t" << var.type_str() << "\t" << var.seq;
	for (unsigned int i=0; i<libs.size(); ++i) {
	  int count = 0;
	  map< string, map< int, map <Variation, int> > >& table = table_libs[i];
	  if (table.find(contig) != table.end()) {
	    map< int, map <Variation, int> >& table_contig = table[contig];
	    if (table_contig.find(coord) != table_contig.end()) {
	      map <Variation, int>& lib_map = table_contig[coord];
	      count = lib_map.find(var) != lib_map.end() ? lib_map[var] : 0;
	    }
	  }
	  out << "\t" << count;
	}
...
int main(int argc, char **argv)
{
  ...
  // lib.index -> contig -> coord -> variation -> count
  vector < map< string, map< int, map <Variation, int> > > > poly_libs(libs.size());

  // lib.index -> contig -> count
  vector < map<string, vector<int> > > coverage_libs(libs.size());

  for (unsigned int i=0; i<libs.size(); ++i) {
    map<string, vector<int> >& coverage = coverage_libs[i];
    for (map<string, int>::iterator it=contig_map.begin(); it!=contig_map.end(); ++it) {
      string contig = (*it).first;
  ...
  for (unsigned int i=0; i<libs.size(); ++i) {
    ...
    map< string, map< int, map <Variation, int> > >& poly = poly_libs[i];
    map<string, vector<int> >& coverage = coverage_libs[i];
  ...
  save_output(libs, contig_map, contig_seq, poly_libs, coverage_libs, ofn);
}
