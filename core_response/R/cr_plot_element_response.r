plot.elements.internal=function(labels, fdir, rep.table, element.ids, anchor.table, q50, norm, min.score, disturb.ids, network, compare)
{
    # remove cluster size
    q50 = q50[,-2]
    ...
    get.anchor.response=function(anchors) {
        ...
        rx = log10(q50[match(anchor.ids, q50$cluster), -1])
    ...
    get.response=function(element.id) {
        ...
        element.q50 = log10(unlist(q50[element.id == q50$cluster, -1]))
        element.q50[element.q50<min.score] = min.score
        result$element.response = element.q50

        inetwork = network[network$cluster == element.id,]
        K = dim(inetwork)[1]
        if (K == 0)
            return (result)
        result$K = K
...
plot.shared.elements=function(ifn.sets, ifn.reps, ifn.anchors, ifn.element2anchor, ifn.map, ifn.bottom0, ifn.median, ifn.top100,
    ifn.detection, labels, disturb.ids, fdir)
{
    sets = load.table(ifn.sets)
    element.table = load.table(ifn.element2anchor)
    ...
    # limit to shared
    fc = field.count(element.table, "element.id")
    element.ids = fc$element.id[fc$count>1]

    # limit to defined sets
    element.ids = intersect(element.ids, sets$set)

    # limit to connected elements
    element.ids = intersect(element.ids, network$element.id)

    plot.elements.internal(labels=labels, fdir=fdir, rep.table=rep.table, disturb.ids=disturb.ids, compare=F,
                           element.ids=element.ids, anchor.table=anchor.table, q50=q50, min.score=min.score, network=network)
}

plot.elements.compare=function(ifn.sets, ifn.reps, ifn.anchors, ifn.map, ifn.bottom0, ifn.median, ifn.top100,
    ifn.detection, labels, disturb.ids, fdir)
{
    system(paste("rm -rf", fdir))
    ...
    # limit to changes
    element.ids = unique(network$cluster[network$type != "unknown"& network$type != "stable"])
    ...
    plot.elements.internal(labels=labels, fdir=fdir, rep.table=rep.table, disturb.ids=disturb.ids, compare=T,
                           element.ids=element.ids, anchor.table=anchor.table, q50=q50, min.score=min.score, network=network)
}
