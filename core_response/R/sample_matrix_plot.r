plot.sample.matrix=function(ifn.bins, ifn.order, ifn.response, ifn.contigs, ifn.median, labels, disturb.ids, fdir)
{
    ...
    patterns = patterns[,-2]
    patterns = patterns[is.element(patterns$cluster, anchor.table$id),]

    anchor.ids = anchor.table$id[is.element(anchor.table$id,patterns$cluster)]
    ...
    for (anchor.id in anchor.ids) {
        anchor.bins = bins$bin[bins$set == anchor.id]
        mat = response[is.element(response$bin,anchor.bins),-1]
