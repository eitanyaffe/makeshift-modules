plot.mkt=function(ifn1, ifn2, max.poly.density, years, fdir)
{
    ...
    df$col.cross = ifelse(df$index == "all", "red", "black")
    ...
    plot.f=function(title, post.f) {
        fig.start(fdir=fdir, ofn=paste(fdir, "/", title, ".pdf", sep=""), type="pdf", height=5.5, width=5)
        plot.init(xlim=xlim, ylim=ylim, xlab="pN/pS", ylab="dN/dS", main=N)
        grid()
