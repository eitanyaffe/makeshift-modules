plot.matrix=function(
    smat, field, anchors, ids, add.text.x=F, add.text.y=T, default.value, mai, breaks, colors, rotated, matrix.border,
    add=F, diag.style="both")
{
    N = length(anchors)
    dim = c(N, N)
    ...
    if (!rotated) {
        if (!add) plot.window(xlim=lim, ylim=lim)
        ...
        xlim = lim
        ylim = xlim/2
        if (!add) plot.window(xlim=xlim, ylim=ylim, yaxs="i")
...
plot.bar=function(x, field, anchors, ids, col="blue", ylim, add.axis=T, axis.at=NULL, axis.labels=NULL, mai, label, ybottom=0,
    ytop=NULL, axis.side=2, cex.axis=0.75)
{
    x$coord = match(x$anchor, anchors) - 0.5
    N = length(anchors)
    coords = 0:N
    ...
    if (add.axis) {
        if (!is.null(axis.at))
            axis(axis.side, at=axis.at, labels=axis.labels, las=2, cex.axis=cex.axis)
        else
            axis(axis.side, las=2, cex.axis=cex.axis)
    }
}
...
plot.taxa=function(taxa, anchors, ids, mai, axis.side, plot.genus=T, gap=0.05)
{
    taxa$letter[taxa$letter=="-"] = ""
    ...
    if (plot.genus) text(x=coords+0.5, y=0.5, taxa$letter[ix], cex=1.2, adj=c(0.5, 0.5))
...
plot.complete.basic=function(
    contig.ifn, ca.ifn, coverage.ifn, gc.ifn, mat.ifn, order.ifn, info.ifn, group.ifn, taxa.ifn,
    checkm.ifn, prev.ifn, min.identity, fdir)
{
    ...
    if (file.exists(prev.ifn)) {
        prev = load.table(prev.ifn)
        prev$anchor = prev$genome
        prev$subject.percent = prev$subject.ratio * 100
    }
    ...
    identity.breaks = c(30, 50, 60, 98, 100)
    identity.colors = c("darkblue", "blue", "red", "orange", "yellow")
    ...
    for (i in 1:2) {
        ...
        plot.bar(x=checkm, field="Contamination", anchors=anchors, ids=ids, mai=mai, col=color, label="contamination (%)", axis.side=axis.side)

        # big
        if (file.exists(prev.ifn))
            plot.bar(x=prev, field="subject.percent", anchors=anchors, ids=ids, mai=mai, col=color, label="prevalence", axis.side=axis.side)
...
plot.accessory.matrix=function(
    mat.ifn, anchor.info.ifn, order.ifn, host.matrix.ifn, taxa.ifn, core.ifn, min.identity, fdir)
{
    smat = load.table(mat.ifn)
    taxa = load.table(taxa.ifn)
    anchor.table = load.table(order.ifn)
    host.matrix = load.table(host.matrix.ifn)
    cores = load.table(core.ifn)
    ainfo = load.table(anchor.info.ifn)

    anchor.table$anchor = anchor.table$set
    anchor.table$anchor.id = anchor.table$id

    anchor.table = anchor.table[is.element(anchor.table$anchor, cores$anchor),]

    ix = match(anchor.table$anchor, cores$anchor)
    anchor.table$acc.percent = 100 * ifelse(!is.na(ix), ifelse(cores$selected[ix], 1-cores$core.fraction[ix], 0), 0)
    anchor.table$total.genes = ifelse(!is.na(ix), cores$gene.count[ix], 0)

    # append shared
    ainfo$shared.percent = 100 * ainfo$shared.genes / ainfo$gene.count
    ix = match(anchor.table$anchor, ainfo$anchor)
    anchor.table$shared.percent = ainfo$shared.percent[ix]

    host.matrix$set1 = host.matrix$anchor.x
    host.matrix$set2 = host.matrix$anchor.y
    ...
    # identity
    identity.breaks = c(30, 50, 60, 100)
    identity.colors = c("darkblue", "blue", "red", "orange")
    identity.panel = make.color.panel(identity.colors)
    wlegend2(fdir=fdir, panel=identity.panel, breaks=identity.breaks, title="identity_summary")

    # shared genes
    shared.genes.breaks = c(0, 1, 10, 100)
    shared.genes.colors = c("white", "green", "orange", "purple")
    shared.genes.panel = make.color.panel(shared.genes.colors)
    wlegend2(fdir=fdir, panel=shared.genes.panel, breaks=shared.genes.breaks, title="shared_genes")

    # shared elements
    shared.elements.breaks = c(0, 1, 10, 100)
    shared.elements.colors = c("white", "green", "orange", "purple")
    shared.elements.panel = make.color.panel(shared.elements.colors)
    wlegend2(fdir=fdir, panel=shared.elements.panel, breaks=shared.elements.breaks, title="shared_elements")

    plot.internal=function(field, shared.breaks, shared.colors) {
        fig.start(fdir=fdir, ofn=paste(fdir, "/summary_", field, ".pdf", sep=""), type="pdf", height=10, width=13)
        mai = c(0.01,3,0.01,2)
        par(xaxs="i")
        ...
        layout(matrix(1:N, N, 1), heights=c(height.matrix, rep(height.small,N.small), height.taxa, height.names))

        # matrix
        plot.matrix(smat=smat, anchors=anchors, ids=ids, add.text.y=F, mai=mai, matrix.border="lightgray", rotate=F,
                    diag.style="up", field="identity", breaks=identity.breaks, colors=identity.colors, default.value=min.identity)
        plot.matrix(smat=host.matrix, anchors=anchors, ids=ids, add.text.y=F, mai=mai, matrix.border="lightgray", rotate=F,
                    diag.style="down", field=field, breaks=shared.breaks, colors=shared.colors, default.value=0, add=T)
        abline(a=0,b=1)
        rect(0,0,A,A)

        plot.bar(x=anchor.table, field="acc.percent", anchors=anchors, ids=ids, mai=mai, col="darkblue", label="%", cex.axis=1)
        plot.bar.append(x=anchor.table, field="shared.percent", anchors=anchors, col="red", ybottom=0)

        # small
        plot.taxa(taxa=taxa, anchors=anchors, ids=ids, mai=mai, axis.side=4, plot.genus=F)

        # small
        plot.names(anchors=anchors, ids=ids, mai=mai, cex=1.4)

        fig.end()
    }

    plot.internal(field="gene.count", shared.breaks=shared.genes.breaks, shared.colors=shared.genes.colors)
    plot.internal(field="element.count", shared.breaks=shared.elements.breaks, shared.colors=shared.elements.colors)
}
