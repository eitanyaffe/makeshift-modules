taxa.path=function(ifn.order, ifn.tree, ifn.taxa, ifn.gene.table, ifn.summary, ofn.path, ofn.species)
{
	...
	name=tt$name,
        ...
        is.env=F, is.cag=F, cag.name="none", cag.level="none")
    for (i in 1:length(anchor.ids)) {
        ...
        if (is.cag) {
            ...
            if (taxa$name[match(pid, taxa$tax_id)] != "environmental samples")
                cat(sprintf("Warning: expecting 'environmental samples' as the parent of CAG %s, found: %s\n", anchor, taxa$name[match(pid, taxa$tax_id)]))
            df.species$cag.level[i] = taxa$level[match(genus.id, taxa$tax_id)]
            df.species$cag.name[i] = taxa$name[match(genus.id, taxa$tax_id)]
        }

        # is env
        ix = df.path$anchor == anchor
        df.species$is.env[i] = any(df.path$level[ix] == "env.genus") || any(df.path$name[ix] == "environmental samples" || is.cag)

        # add coverage
        gene.total = sum(summary[match(anchor, summary$anchor),-1])
        df.species$coverage[i] = round(100 * df.species$gene_count[i] / gene.total,2)
    }
