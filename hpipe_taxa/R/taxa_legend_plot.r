...
plot.tree.wrapper=function(fdir, tree, title, height, width, pnode, pnode.text, mai=c(1,0,0.5,0.5)) {
...
plot.taxa.legend.color=function(ifn.taxa, ifn.legend.color, fdir)
{
    ...
    plot.tree.wrapper(fdir=fdir, tree=tree, pnode=pnode, pnode.text=pnode.text, title="color", height=6, width=10, mai=c(1,0,0.5,2))
...
plot.accessory.legend=function(ifn.cores, ifn.taxa, ifn.legend, ifn.legend.color, fdir)
{
    taxa = load.table(ifn.taxa)
    legend = load.table(ifn.legend)
    cores = load.table(ifn.cores)
    df = load.table(ifn.legend.color)

    legend = legend[is.element(legend$anchor,cores$anchor),]

    tree = make.legend.tree(taxa=taxa, df=df, default.value="lightgray")
    lo = leaf.order(tree)

    tt = table(legend$group.id)
    df = data.frame(id=names(tt), count=as.vector(tt))
    df$group.name = taxa$name[match(df$id,taxa$tax_id)]
    df$index = lo$index[match(df$id, lo$id)]
    df$color = legend$color[match(df$id,legend$group.id)]
    df$title = paste(df$group.name, " (", df$count, ")", sep="")
    df = df[order(df$index, decreasing=T),]
    unknown = paste("unknown (", sum(is.na(legend$group.id)), ")", sep="")
    wlegend(fdir, names=c(df$title, unknown), cols=c(df$color, "gray"), title="family")
}
