...
genome.table=function(ifn.species, ifn.taxa, ifn.gene.table, only.complete.threshold, max.refs.per.taxa, ofn)
{
    ...
    append.id=function(anchor, rid, id, only.complete) {
        ...
        if (any(ix)) {
            ...
            ttable = ttable[order(
                ttable$assembly_level == "Complete Genome",
                ttable$genome_rep == "Full",
                ttable$release_type == "Major",
                ttable$version_status == "latest",
                decreasing=T),]
            ...
            if (only.complete && dim(df)[1] > max.refs.per.taxa)
                df = df[1:max.refs.per.taxa,]
