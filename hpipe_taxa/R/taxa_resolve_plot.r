...
plot.details=function(df, fdir)
{
    ...
    # colors = c("white", "blue", "red", "orange", "yellow")
    # breaks = c(0, 60, 70, 80, 100)

    breaks = c(30, 50, 60, 98, 100)
    colors = c("darkblue", "blue", "red", "orange", "yellow")
...
{
    df = load.table(ifn)
    df$title = paste(df$short.level, " | ", df$tax.id, " | ", df$name, " | ", df$type)
    plot.details(df=df, fdir=fdir)
}

plot.details.ref=function(ifn, min.cov, min.identity, fdir)
{
    df = load.table(ifn)
    df$class = ifelse(df$identity < min.identity | df$frac < min.cov, "none", ifelse(df$is.env, "env", "culture"))
    df$title = paste(df$class, df$name, sep=" | ")
    plot.details(df=df, fdir=fdir)


    tt = table(df$class)
    fig.start(fdir=fdir, ofn=paste(fdir, "/breakdown.pdf", sep=""), type="pdf", width=2.5, height=4)
    par(mai=c(1,1,1,0.5))
