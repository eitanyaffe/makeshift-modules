seq.summary=function(order.ifn, table.ifn, gene.table.ifn, anchor2ref.dir, ref2anchor.dir, length, ofn, ofn.unique)
{
    ...
    get.coverage=function(ifn) {
        tt = read.delim(ifn)
        round(100*sum(tt$count[tt$edit != -1]) / sum(tt$count),4)
    }
    get.identity=function(ifn) {
        ...
        round(sum(tt$identity*tt$weight),4)
    }
    ...
    result$anchor.coverage = round((result$src.anchor.coverage + result$tgt.anchor.coverage)/2,4)
    result$ref.coverage = round((result$src.ref.coverage + result$tgt.ref.coverage)/2,4)
    result$anchor.identity = round((result$src.anchor.identity + result$tgt.anchor.identity)/2,4)
    result$ref.identity = round((result$src.ref.identity + result$tgt.ref.identity)/2,4)
