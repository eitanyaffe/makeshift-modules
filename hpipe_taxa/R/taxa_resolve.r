...
taxa.resolve=function(ifn.order, ifn.tree, ifn.path, ifn.taxa, ifn.summary, min.coverage, min.ratio, ofn)
{
    ...
    for (anchor.id in ids) {
        ...
        is.env = any(apath$name == "environmental samples")
        ...
        # go up if no rank
        if (apath$level[ix] == "no rank" || apath$level[ix] == "env.genus") {
        ...
        df = data.frame(anchor.id=anchor.id, anchor=anchor, tax.id=rid, violating.id=violating.id,
            ...
            level=apath$level[ix], short.level=short.rank(apath$level[ix]), name=apath$name[ix], depth=depth, is.env=is.env)
